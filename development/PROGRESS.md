...
| 10 | Spec-decode e2e (draft+target) after #8 | HIGH | **NEG** | lossless PASS but 0.36-0.64× — gemm_mma small-M (seq≤7) fixed-cost floor (verify=4.6-5.8× seq1); break-even unreachable |
| 42 | Reduce gemm_mma small-M (seq 2-8) fixed cost | HIGH | TODO | resurrects spec-decode #10 + short-prefill; the verify-forward kernel floor |
