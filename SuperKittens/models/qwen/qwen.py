...
QWEN_ABI = {
    ...
    "set_lm_head_all_rows": optional([ctypes.c_void_p, ctypes.c_uint32], ctypes.c_int),
}
...
class Qwen(Model):
    ...
    def get_logits_rows(self, n_rows: int) -> np.ndarray:
        ...
        recent forward. Spec-decode verification reads all K+1 verify rows.
        Requires set_lm_head_all_rows(True) so the head projected those rows.
        """
        lib = _load()
    ...
    def set_lm_head_all_rows(self, on: bool) -> None:
        lib = _load()
        if not hasattr(lib, "sk_qwen_set_lm_head_all_rows"):
            raise RuntimeError("libsk.dylib has no sk_qwen_set_lm_head_all_rows symbol; rebuild dylib")
        rc = lib.sk_qwen_set_lm_head_all_rows(self._h, ctypes.c_uint32(1 if on else 0))
        if rc: raise RuntimeError(f"sk_qwen_set_lm_head_all_rows failed: {rc}")
