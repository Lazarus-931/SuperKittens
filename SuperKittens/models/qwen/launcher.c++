...
// entries past prev_pos+J must be discarded by rewinding current_pos so the
// next forward overwrites them. get/set pair exposes the cursor for that.
...
// 0..last_seq-1 of bufs.logits; spec-decode verification needs every row from
// the K-token forward, not just the last. Copies the first n_rows rows.
