...
// WHY: spec-decode verification needs per-position logits for all K verify tokens.
int  sk_qwen_get_logits_rows(sk_qwen_handle* h, void* out_fp16, uint32_t n_rows);

// KV-cache cursor get/set. WHY: spec-decode rewinds the cursor to discard the
// KV entries of rejected verify tokens. get returns current_pos (>=0); set
// returns -2 if pos > cache_max.
