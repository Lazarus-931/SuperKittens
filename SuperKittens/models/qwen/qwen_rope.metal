...
void qwen_rope_qk(
    ...
    constant uint& n_rows   [[buffer(7)]],  // batch*seq; ==seq for batch=1 (M=1/prefill)
    ...
    uint3 tpg [[threads_per_threadgroup]])
{
    ...
    // Batched decode lays Q/K as [batch*seq, n_heads, D]; the rotation position
    // is the intra-request seq index (row % seq), the same value all batch lanes
    // share at a given decode step. At batch=1 this is row (unchanged).
    const uint pos_row = row % seq;
    ...
    const size_t cs_off = (size_t)pos_row * hd + i;
...
void qwen_rope_qk_interleaved(
    ...
    constant uint& head_dim [[buffer(5)]],
    constant uint& n_heads  [[buffer(6)]],
    constant uint& n_rows   [[buffer(7)]],  // batch*seq; ==seq for batch=1
    uint3 gid [[threadgroup_position_in_grid]],
    uint3 tid [[thread_position_in_threadgroup]],
    uint3 tpg [[threads_per_threadgroup]])
{
    ...
    const size_t cs_off = (size_t)pos_row * hd + k0;
