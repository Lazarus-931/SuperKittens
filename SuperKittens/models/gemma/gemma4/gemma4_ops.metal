    ...
    uint  tid  [[thread_index_in_threadgroup]])
{
    ...
    y[row_off + tid] = bfloat(xv * inv_rms);
}
...
void gemma4_ple_context_mix(
    device const bfloat*  proj           [[buffer(0)]],  // (T, n_layers, ple_dim)
    device const bfloat*  gamma          [[buffer(1)]],  // (ple_dim,)
    device       bfloat*  per_layer_in   [[buffer(2)]],  // (T, n_layers, ple_dim) in/out
    ...
    uint  tid  [[thread_index_in_threadgroup]])
{
    ...
    per_layer_in[row_off + tid] = (bfloat)out;
}
    ...
    uint gid [[thread_position_in_grid]])
{
    const uint i = gid * 4;
    if (i + 4 <= n) {
        device bfloat4* p = reinterpret_cast<device bfloat4*>(logits + i);
        float4 v = float4(*p);
        float inv = 1.0f / cap;
        ...
        *p = bfloat4(t * cap);
        ...
        for (uint j = i; j < n; ++j) {
            ...
            logits[j] = bfloat(precise::tanh(v / cap) * cap);
        }
    }
}
    ...
    uint gid [[thread_position_in_grid]])
{
    ...
    if (i + 4 <= n) {
        device bfloat4* p = reinterpret_cast<device bfloat4*>(logits + i);
        *p = bfloat4(float4(*p) * inv_scale);
    } else {
        for (uint j = i; j < n; ++j) {
            logits[j] = bfloat((float)logits[j] * inv_scale);
        }
    }
}
