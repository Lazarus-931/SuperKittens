...
static bool resolve_psos(ModelPSOs& P) {
    ...
    P.layer.rmsnorm_t1     = sk::bindings_pso("rmsnorm_bf16_t1");  // optional T=1 fast path (bf16)
...
extern "C" sk_gemma4_handle* sk_gemma4_create(const sk_gemma4_config* cfg) {
    ...
    // Q4_K LM head (SK_GEMMA4_LMHEAD_Q4K; default ON, =0 to disable). The head is
    // the one large-N bandwidth-bound decode matvec, so Q4_K over Q8_0 is half the
    // head bytes = a direct decode win at no measured coherence cost. Validated:
    // E2B 48/48 token-match (#90); 12B-unified 8/8 first-step top-8 logit overlap
    // + coherent over 40 tok, +6.5% per-tok decode and -0.5 GB resident vs Q8 head.
    // Separate buffer from the tied embed Q8 (no Q4_K embed-gather kernel; keep the
    // input side clean). weights.c++ quantizes the scaled bf16 head into it.
    // d_model must be 256-block aligned (E2B/E4B 2560, 12B-unified 3840 all align).
    {
        const char* env = std::getenv("SK_GEMMA4_LMHEAD_Q4K");
        const bool want_q4k = !env || env[0] != '0';
    ...
    }
    {
        ...
        // When the Q4_K head is on it serves the head matvec, so the Q8 head is
        // only still needed for the tied embed lookup (embed_q8). Skip the
        // redundant 0.43 GB Q8 head alloc otherwise.
        bool want_q8 = embed_q8 || !env || (env[0] != '0');
        if (h->weights.w_lm_head_q4k && !embed_q8) want_q8 = false;
...
extern "C" int sk_gemma4_dump_layer(sk_gemma4_handle* hp, const char* name, void* out_fp16) {
    ...
    // L0-extra slots (pre/post-RoPE q/k, attn-pre): laid out after logits.
    {
        ...
        // L0 PLE probes laid out after L0.pre_ple slot.
        {
            const size_t qkv_slots_max_e = H_n + 2u * n_kv_max_e;
            const size_t off_pre_ple     = off_ap + H_n * hd_max_e + qkv_slots_max_e * hd_max_e;
            const size_t off_ple_gate    = off_pre_ple  + h->cfg.d_model;
            const size_t off_ple_gated   = off_ple_gate + h->cfg.ple_dim;
            const size_t off_ple_proj    = off_ple_gated+ h->cfg.ple_dim;
