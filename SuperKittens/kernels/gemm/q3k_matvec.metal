...
//
// Fit-enabler, not a decode-speed win: a native Q3_K dot keeps Q3_K projections
// (Q3_K_M / Q3_K_S recipes store most projections as Q3_K ≈ 3.44 bpw) packed
// end-to-end instead of host-dequant'ing to fp16 (16 bpw), which is what makes
// sub-Q4 weight-quant recipes fit a larger model in 16 GB. Q3_K decode is
// settled-NEG for tok/s on M4 base (compute-bound vs the bandwidth-bound q4k),
// so do NOT route decode through this for perf — it exists to fit, not to win.
//
// Block layout (GGML block_q3_K, 110 B / 256 weights; per llama.cpp
// dequantize_row_q3_K, validated against gguf.quants Q3_K dequant):
...
// CLEAR → subtract 4. w = d * (scale[n>>4]-32) * (q2 - hbit).
//
// Binding + dispatch mirror q4k/q6k/q5k_matvec so the launcher picks the PSO by
// dtype with one uniform call shape:
//   NW=32 lanes, NSG=4 simdgroups tile the K (super-block) dimension, NR0=2
//   output rows per TG. Lane l owns pos=l; the 32 lanes tile the 32 pos values
//   of a super-block once, each accumulating all 8 (group,shift) taps — so one
//   simdgroup covers all 256 weights of a block per step.
//
// Bindings: 0 B act fp16[K]  1 A weights q3_K[N,K]  2 C out fp16[N]  3 uint K
//           4 uint N
    ...
    ushort  sgitg [[simdgroup_index_in_threadgroup]])
{
    ...
    for (uint ib = sgitg; ib < nb; ib += Q3_NSG) {
        ...
        for (short r = 0; r < Q3_NR0; ++r) {
            ...
            // Unpack the 16 signed 6-bit sub-scales (offset -32) once per block.
            // scales lives at struct offset 96 with a 110 B stride — for odd ib
            // that address is only 2-byte aligned, so a uint32 (or uint16) load
            // there is misaligned UB on Metal. Assemble the aux words from
            // individual bytes instead (llama.cpp's get_scale_min_k4 shuffle).
            uint32_t aux[4];
            {
                device const uint8_t *sb = ax[r][ib].scales;
                const uint32_t a0 = (uint32_t)sb[0] | ((uint32_t)sb[1] << 8) | ((uint32_t)sb[2]  << 16) | ((uint32_t)sb[3]  << 24);
                const uint32_t a1 = (uint32_t)sb[4] | ((uint32_t)sb[5] << 8) | ((uint32_t)sb[6]  << 16) | ((uint32_t)sb[7]  << 24);
                const uint32_t a2 = (uint32_t)sb[8] | ((uint32_t)sb[9] << 8) | ((uint32_t)sb[10] << 16) | ((uint32_t)sb[11] << 24);
            ...
            // Each lane uses two scale indices per group (pos<16 vs pos>=16).
