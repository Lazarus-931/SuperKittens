...
from kernels.gemm.baseline.mlx.gemm import gemm_bias_silu

_INPUTS_KEY = "_cached_inputs"


def _make_inputs(cfg: dict) -> tuple[mx.array, mx.array, mx.array, mx.array]:
    ...
    bias = mx.random.normal((n,), dtype=dtype)
    return a, b, c_in, bias


def _get_inputs(cfg: dict) -> tuple[mx.array, mx.array, mx.array, mx.array]:
...
def _flops(cfg: dict) -> float:
    ...
    n = int(cfg["n"])
    k = int(cfg["k"])
    return 2.0 * m * n * k + 6.0 * m * n


@bench("gemm_mlx_bias_silu", flops=_flops)
def gemm_mlx_bias_silu(cfg: dict):
    a, b, c_in, bias = _get_inputs(cfg)
    return gemm_bias_silu(a, b, bias, c_in, float(cfg.get("alpha", 1.0)), float(cfg.get("beta", 0.0)))
