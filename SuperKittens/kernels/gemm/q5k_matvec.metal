...
//
// The Q3_K_M recipe bumps a couple of early layers' attn_v / ffn_down to Q5_K
// (llama.cpp's "use_more_bits" for the first layers). The split-V dispatch has
// no fp16 path, so V must be a native-kernel dtype — this keeps those Q5_K
// tensors packed (176 B / 256 ≈ 5.5 bpw) instead of erroring / host-dequanting.
//
...
//   NW=32, NSG=4 simdgroups tile K, NR0=2 rows/TG. Bindings mirror q4k/q6k:
//   0:B act fp16[K]  1:A weights q5_K[N,K]  2:C out fp16[N]  3:uint K  4:uint N
