...
// dk=192 (DS4 MLA shape): trailing param is NE, the "elements per lane"
// divisor. NL = N_SIMDWIDTH/NE = 32/NE must divide DK/4. For dk=192 → DK4=48,
// we need NL ∈ {1,2,3,4,6,8,12,16,24,48} ∩ {1,2,4,8,16,32} = {1,2,4,8,16},
// so NE ∈ {2,4,8,16,32}. The existing 128/512 instantiations work at NE=1
// because DK4 ∈ {32,128} are multiples of 32. dk=192 needs NE=4 (NL=8).
template [[host_name("kernel_flash_attn_ext_vec_f16_dk192_dv128")]]  kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES,     half4,  1, dequantize_f16_t4, half4,  1, dequantize_f16_t4, 192, 128, 4>;
