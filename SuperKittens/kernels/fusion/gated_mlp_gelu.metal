...
[[host_name("gated_mlp_gelu")]]
[[kernel, max_total_threads_per_threadgroup(THREADS)]]
void gated_mlp_gelu(
    device const half* x,          // (M, K)
    device const half* w_gate,     // (K, N_int)
    device const half* w_up,       // (K, N_int)
    device const half* w_down,     // (N_int, N)
    device half* out,              // (M, N)
    ...
    uint2 gid [[threadgroup_position_in_grid]])
{
    ...
    threadgroup half As[BM * BK];          // 4 KB — reused across passes
    threadgroup half Bs[BK * BN];          // 4 KB — reused across passes
    threadgroup half intermediate[BM * BN]; // 8 KB — one BN-chunk of GeGLU output
    ...
    for (uint nint_chunk = 0; nint_chunk < N_int; nint_chunk += BN) {
        ...
        for (uint r = 0; r < MR; r++)
            for (uint c = 0; c < MC; c++) {
                ...
                intermediate[(gr_abs % BM) * BN + gc_local]
                    = valid ? half(gelu_approx(g.x) * up_v.x) : half(0);
                intermediate[(gr_abs % BM) * BN + gc_local + 1]
                    = valid1 ? half(gelu_approx(g.y) * up_v.y) : half(0);
            }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        for (uint bk = 0; bk < BN; bk += BK) {
            for (uint i = tid; i < (BM * BK) / 4; i += THREADS) {
                ...
                reinterpret_cast<threadgroup half4*>(As)[i] =
                    (gr_local < BM)
                        ? reinterpret_cast<threadgroup half4*>(intermediate + gr_local * BN + bk)[c / 4]
                        : half4(0);
            }
            for (uint i = tid; i < (BK * BN) / 4; i += THREADS) {
                uint r = (i * 4) / BN, c = (i * 4) % BN;
    ...
    for (uint r = 0; r < MR; r++)
        for (uint c = 0; c < MC; c++) {
            ...
            uint gc = col + c * 8 + lc;
            if (gr < M && gc < N) {
                out[(size_t)gr * N + gc] = half(v.x);
                if (gc + 1 < N)
                    out[(size_t)gr * N + gc + 1] = half(v.y);
            }
        }
}
