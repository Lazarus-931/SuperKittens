...
extern "C" int sk_moe_down_scatter_q2k(void* hidden, void* W_down, void* exp_ids,
                                       void* route_w, void* residual, void* out,
                                       uint32_t T, uint32_t top_k, uint32_t E,
                                       uint32_t D, uint32_t N_int) {
    auto* pso = sk::bindings_pso("moe_down_scatter_q2k");
    if (!pso) return -1;
    if (N_int % 256) return -4;
    auto* dev = sk::bindings_device();

    // 84 bytes per 256-weight Q2_K block.
    const size_t Q2K_BLOCK_BYTES = 84;
    const size_t n_blocks_per_row = N_int / 256;

    const size_t hb   = (size_t)T  * top_k * N_int * 2;
    const size_t wb   = (size_t)E  * D * n_blocks_per_row * Q2K_BLOCK_BYTES;
