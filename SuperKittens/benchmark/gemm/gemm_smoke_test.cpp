...
//  SuperKittens
//
//  CPU vs GPU smoke test for the FP16 GEMM scaffold.
//
//  Build:
//    clang++ -std=gnu++20 -O2 -arch arm64 -I metal-cpp \
//      -framework Metal -framework Foundation -framework QuartzCore \
//      SuperKittens/benchmark/gemm/gemm_smoke_test.cpp -o build/gemm_smoke_test
//
//  Run:
//    ./build/gemm_smoke_test <metallib> [M=256] [N=256] [K=256]
//
    ...
    const meow::gemm::GemmParams& p)
{
    ...
    for (int row = 0; row < p.m; ++row) {
        for (int col = 0; col < p.n; ++col) {
            ...
            Cout[static_cast<size_t>(row) * p.ldc + col] = p.alpha * acc + p.beta * prior;
        }
    }
}
...
int main(int argc, const char* argv[]) {
    ...
    meow::gemm::GemmParams p = meow::gemm::make_gemm_params(M, N, K);
    if (!force_generic) {
    ...
    std::mt19937 rng(42);
    std::vector<__fp16> A(a_elems), B(b_elems), C0(c_elems), Cgpu(c_elems);
