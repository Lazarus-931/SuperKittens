...
int main(int argc, const char* argv[]) {
    ...
    auto* fn = lib->newFunction(NS::String::string("mha_noncausal", NS::UTF8StringEncoding));
    if (!fn) {
        std::fprintf(stderr, "Kernel 'mha_noncausal' not found\n");
        return 1;
    }
    ...
    attention_cpu(Q.data(), K.data(), V.data(), ref.data(), seq, d);
    ...
    std::printf("== noncausal mha baseline ==\n");
