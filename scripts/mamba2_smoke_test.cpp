...
//      scripts/mamba2_smoke_test.cpp -o build/mamba2_smoke_test
//
//  Run:
...
struct ErrStats {
    float max_abs = 0.0f, mean_abs = 0.0f, l2_rel = 0.0f;
};
...
int main(int argc, const char* argv[]) {
    ...
    int L = (argc > 2) ? std::atoi(argv[2]) : 256;
    int B = (argc > 3) ? std::atoi(argv[3]) : 1;
    int H = (argc > 4) ? std::atoi(argv[4]) : 2;
    std::string dump_dir;
    for (int i = 5; i + 1 < argc; ++i)
        if (std::string(argv[i]) == "--dump") dump_dir = argv[i + 1];

    constexpr int CHUNK = 32, D_QK = 64, D_V = 64;
    if (L % CHUNK != 0) {
    ...
    if (!fn) { std::fprintf(stderr, "Kernel '%s' not found\n", name); return 1; }
    ...
    auto run = [&]() -> double {
        ...
        enc->setBuffer(bA, 0, 3); enc->setBuffer(bO, 0, 4); enc->setBuffer(bArgs, 0, 5);
    ...
    bQ->release(); bK->release(); bV->release(); bA->release();
    bO->release(); bArgs->release();
